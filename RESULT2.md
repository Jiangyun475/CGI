...
| 配置 | Fold0 AUC | Δ vs base(CL) | Δ vs no_CL | 状态 |
|------|-----------|--------------|-----------|------|
| base（CL, 官方基准） | **0.8941** | — | +0.0016 | ✅ |
| no_CL（消融实验一） | 0.8925 | −0.0016 | — | ✅ |
| +SBB | 0.8917 | −0.0024 | −0.0008 | ✅ |
| +Tanimoto | 0.8916 | −0.0025 | −0.0009 | ✅ |
| +Both | **0.8924** | −0.0017 | −0.0001 | ✅ |

**结论：两个创新均未改善性能。**
- SBB：强制共享查询约束了异质模态各自的表示自由度，隐式对齐已足够。
- Tanimoto：Tanimoto ≠ 生物程序相似性，在多跳 CGI 任务中是错误先验（残余损失 ≈0.095 非零证明无法消除冲突）。
- Both：两者相互部分抵消，接近 no_CL 水平（−0.0001），说明两个失败有不同方向的偏差。
