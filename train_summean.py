#!/usr/bin/env python3
"""
SumMean + Ortho + CL 训练脚本
==============================
架构：GINLayer(edge_attr) × 3  →  sum/mean pool  →  Ortho剥离  →  FocalCL + BCE

与 train_ultimate.py 的区别：
  - 无 JK（Jumping Knowledge）
  - 无 Entmax（回退为标准 softmax，且 sum_mean 根本不用 attention）
  - 无 context_mlp / attn_proj
  - pool_type 固定为 sum_mean
  - 其余训练超参、数据加载、Loss 结构完全一致
"""
...
# ================================================================
# 0. 工具函数 & 数据集（与 train_ultimate.py 完全一致）
# ================================================================
...
class ChemEncoder(nn.Module):
    """
    GINLayer × 3  →  sum_pool + mean_pool  →  readout
    不含 JK、不含 attention、不含 context_mlp。
    """
    def __init__(self, hidden_dim=128, dropout=0.3):
        ...
        # sum + mean → 2 × hidden_dim → hidden_dim
    ...
    def forward(self, x, edge_index, edge_attr, num_nodes_list):
        ...
        h_c = torch.cat([sum_pool, mean_pool], dim=1)  # [B, 2H]
        return self.readout(h_c)                        # [B, H]
...
def train(args):
    ...
    save_dir = Path(f'results_summean/{Path(args.data_dir).name}')
    save_dir.mkdir(parents=True, exist_ok=True)
    tag = f"_{args.run_tag}" if args.run_tag else ""
    model_name = f"summean_ortho_Fold{args.fold}{tag}.pt"

    print(f"\n🚀 SumMean+Ortho+CL | Device: {args.device} | Fold: {args.fold} | AMP: {args.use_amp}")
